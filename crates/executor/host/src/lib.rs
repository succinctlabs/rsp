...
use rsp_client_executor::{
    io::ClientExecutorInput, ChainVariant, EthereumVariant, LineaVariant, OptimismVariant, SepoliaVariant, Variant, CliqueShanghaiChainIDVariant, VariantChainID
};
...
impl<T: Transport + Clone, P: Provider<T, AnyNetwork> + Clone> HostExecutor<T, P> {
    ...
    pub async fn execute(
        ...
        let client_input = match variant {
            ...
            ChainVariant::CliqueShanghaiChainID => self.execute_variant_with_chain_id::<CliqueShanghaiChainIDVariant>(self.provider.get_chain_id().await?, block_number).await,
        }?;
    ...
    async fn execute_variant_with_chain_id<V>(&self, chain_id: u64, block_number: u64) -> eyre::Result<ClientExecutorInput>
    where
        V: VariantChainID,
    {
        // Fetch the current block and the previous block from the provider.
        ...
        let executor_block_input = V::pre_process_block(&current_block)
            ...
            .ok_or(eyre!("failed to recover senders"))?;
        let executor_difficulty = current_block.header.difficulty;
        let executor_output = V::execute(&executor_block_input, executor_difficulty, cache_db, chain_id)?;

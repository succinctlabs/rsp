...
use rsp_client_executor::{
    io::ClientExecutorInput, ChainVariant, EthereumVariant, ImmutableVariant, ImmutableTestnetVariant, LineaVariant, OptimismVariant, Variant,
};
...
impl<T: Transport + Clone, P: Provider<T, AnyNetwork> + Clone> HostExecutor<T, P> {
    ...
    pub async fn execute(
        ...
        let client_input = match variant {
            ...
            ChainVariant::Immutable => self.execute_variant::<ImmutableVariant>(block_number).await,
            ChainVariant::ImmutableTestnet => self.execute_variant::<ImmutableTestnetVariant>(block_number).await,
        }?;

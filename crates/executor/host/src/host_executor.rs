...
impl<C: ConfigureEvm, CS> HostExecutor<C, CS> {
        ...
        N: Network,
    {
        ...
        let header = Header {
            ...
            // EIP-7928 (Amsterdam BAL) + EIP-7912 slot-number — None pre-fork, passed through.
            // Method-call form here (vs. field access in the client) because the header here
            // is the `BlockHeader` trait projection, not the concrete `alloy_consensus::Header`.
            block_access_list_hash: current_block.header().block_access_list_hash(),
            slot_number: current_block.header().slot_number(),
        };
        ...
        // Re-encode the state into the arena witness blob so the guest can decode it
        // zero-copy (borrowing the input bytes directly).

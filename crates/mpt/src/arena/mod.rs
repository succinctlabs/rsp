...
//!
//! * stores every node in a flat `Vec`, referencing children by `u32` index (cache-friendly,
//!   no per-node `Box` allocation);
//! * borrows leaf/extension paths and values directly from the input buffer or a `bumpalo`
//!   bump arena (zero-copy), instead of owning heap `Vec`s;
